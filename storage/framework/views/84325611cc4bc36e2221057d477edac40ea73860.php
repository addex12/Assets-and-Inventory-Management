...
<script nonce="<?php echo e(csrf_token()); ?>">
    $(function () {
        ...
        var locale = '<?php echo e(config('app.locale')); ?>';
        ...
        $('.snipe-table').bootstrapTable('destroy').each(function () {
            ...
            formatLoadingMessage: function () {
                return '<h2><i class="fas fa-spinner fa-spin" aria-hidden="true"></i> <?php echo e(trans('general.loading')); ?> </h2>';
            },
            icons: {
    ...
    function genericColumnObjLinkFormatter(destination) {
        return function (value,row) {
            if ((value) && (value.status_meta)) {
                ...
                return '<nobr><a href="<?php echo e(config('app.url')); ?>/' + dpolymorphicItemFormatterest + dest + '/' + value.id + '">' + value.name + '</a></span>';
            }
        };
    }

    function hardwareAuditFormatter(value, row) {
        return '<a href="<?php echo e(config('app.url')); ?>/hardware/audit/' + row.id + '/" class="btn btn-sm bg-yellow" data-tooltip="true" title="Audit this item"><?php echo e(trans('general.audit')); ?></a>';
    }
    ...
    function genericActionsFormatter(owner_name, element_name) {
        ...
        return function (value,row) {
            ...
            if ((row.available_actions) && (row.available_actions.clone === true)) {
                actions += '<a href="<?php echo e(config('app.url')); ?>/' + dest + '/' + row.id + '/clone" class="actions btn btn-sm btn-info" data-tooltip="true" title="<?php echo e(trans('general.clone_item')); ?>"><i class="far fa-clone" aria-hidden="true"></i><span class="sr-only">Clone</span></a>&nbsp;';
            }

            if ((row.available_actions) && (row.available_actions.update === true)) {
                actions += '<a href="<?php echo e(config('app.url')); ?>/' + dest + '/' + row.id + '/edit" class="actions btn btn-sm btn-warning" data-tooltip="true" title="<?php echo e(trans('general.update')); ?>"><i class="fas fa-pencil-alt" aria-hidden="true"></i><span class="sr-only"><?php echo e(trans('general.update')); ?></span></a>&nbsp;';
            } else {
                if ((row.available_actions) && (row.available_actions.update != true)) {
                    actions += '<span data-tooltip="true" title="<?php echo e(trans('general.cannot_be_edited')); ?>"><a class="btn btn-warning btn-sm disabled" onClick="return false;"><i class="fas fa-pencil-alt"></i></a></span>&nbsp;';
                }
            }
            ...
            if ((row.available_actions) && (row.available_actions.delete === true)) {
                ...
                actions += '<a href="<?php echo e(config('app.url')); ?>/' + dest + '/' + row.id + '" '
                    ...
                    + '<i class="fas fa-trash" aria-hidden="true"></i><span class="sr-only"><?php echo e(trans('general.delete')); ?></span></a>&nbsp;';
            } else {
                // Do not show the delete button on things that are already deleted
                if ((row.available_actions) && (row.available_actions.restore != true)) {
                    actions += '<span data-tooltip="true" title="<?php echo e(trans('general.cannot_be_deleted')); ?>"><a class="btn btn-danger btn-sm delete-asset disabled" onClick="return false;"><i class="fas fa-trash"></i></a></span>&nbsp;';
                }
            ...
            if ((row.available_actions) && (row.available_actions.restore === true)) {
                ...
                actions += '<button class="btn btn-sm btn-warning" data-tooltip="true" title="<?php echo e(trans('general.restore')); ?>"><i class="fas fa-retweet"></i></button>&nbsp;';
            }
    ...
    function licenseSeatInOutFormatter(value, row) {
        ...
        if ((row.available_actions.checkout === true) && (row.user_can_checkout === true) && ((!row.asset_id) && (!row.assigned_to))) {
            ...
            return '<a href="<?php echo e(config('app.url')); ?>/licenses/' + row.id + '/checkin" class="btn btn-sm bg-purple" data-tooltip="true" title="Check in this license seat."><?php echo e(trans('general.checkin')); ?></a>';
        }
    ...
    function genericCheckinCheckoutFormatter(destination) {
        return function (value,row) {
            ...
            // The user is allowed to check items in
                ...
                if (row.assigned_to) {
                    return '<a href="<?php echo e(config('app.url')); ?>/' + destination + '/' + row.id + '/checkin" class="btn btn-sm bg-purple" data-tooltip="true" title="Check this item in so it is available for re-imaging, re-issue, etc."><?php echo e(trans('general.checkin')); ?></a>';
                } else if (row.assigned_pivot_id) {
                    return '<a href="<?php echo e(config('app.url')); ?>/' + destination + '/' + row.assigned_pivot_id + '/checkin" class="btn btn-sm bg-purple" data-tooltip="true" title="Check this item in so it is available for re-imaging, re-issue, etc."><?php echo e(trans('general.checkin')); ?></a>';
                }
    ...
    function assetRequestActionsFormatter (row, value) {
        if (value.assigned_to_self == true){
            return '<button class="btn btn-danger btn-sm disabled" data-tooltip="true" title="Cancel this item request"><?php echo e(trans('button.cancel')); ?></button>';
        } else if (value.available_actions.cancel == true)  {
            return '<form action="<?php echo e(config('app.url')); ?>/account/request-asset/'+ value.id + '" method="POST"><?php echo csrf_field(); ?><button class="btn btn-danger btn-sm" data-tooltip="true" title="Cancel this item request"><?php echo e(trans('button.cancel')); ?></button></form>';
        } else if (value.available_actions.request == true)  {
            return '<form action="<?php echo e(config('app.url')); ?>/account/request-asset/'+ value.id + '" method="POST"><?php echo csrf_field(); ?><button class="btn btn-primary btn-sm" data-tooltip="true" title="Request this item"><?php echo e(trans('button.request')); ?></button></form>';
        }
            ...
            if ((row.custom_fields) && (row.custom_fields[field_column_plain])) {
                ...
                if ((row.custom_fields[field_column_plain].field_format) && (row.custom_fields[field_column_plain].value)) {
                    if (row.custom_fields[field_column_plain].field_format=='URL') {
                        ...
                        return '<a href="mailto:' + row.custom_fields[field_column_plain].value + '" style="white-space: nowrap" data-tooltip="true" title="<?php echo e(trans('general.send_email')); ?>"><i class="fa-regular fa-envelope" aria-hidden="true"></i> ' + row.custom_fields[field_column_plain].value + '</a>';
                    }
                }
    ...
    function minAmtFormatter(row, value) {
        ...
        if ((row) && (row!=undefined)) {
            if (value.free_seats_count <= value.min_amt) {
                return  '<span class="text-danger text-bold" data-tooltip="true" title="<?php echo e(trans('admin/licenses/general.below_threshold_short')); ?>">' + value.min_amt + '</span>';
            }
    ...
    function phoneFormatter(value) {
        if (value) {
            return  '<span style="white-space: nowrap;"><a href="tel:' + value + '" data-tooltip="true" title="<?php echo e(trans('general.call')); ?>"><i class="fa-solid fa-phone" aria-hidden="true"></i> ' + value + '</a></span>';
        }
    }
    ...
    function assetTagLinkFormatter(value, row) {
        if ((row.asset) && (row.asset.id)) {
            if (row.asset.deleted_at!='') {
                return '<span style="white-space: nowrap;"><i class="fas fa-times text-danger"></i><span class="sr-only"><?php echo e(trans('admin/hardware/general.deleted')); ?></span> <del><a href="<?php echo e(config('app.url')); ?>/hardware/' + row.asset.id + '" data-tooltip="true" title="<?php echo e(trans('admin/hardware/general.deleted')); ?>">' + row.asset.asset_tag + '</a></del></span>';
            }
    ...
    function assetSerialLinkFormatter(value, row) {
        ...
        if ((row.asset) && (row.asset.serial)) {
            if (row.asset.deleted_at!='') {
                return '<span style="white-space: nowrap;"><i class="fas fa-times text-danger"></i><span class="sr-only">deleted</span> <del><a href="<?php echo e(config('app.url')); ?>/hardware/' + row.asset.id + '" data-tooltip="true" title="<?php echo e(trans('admin/hardware/general.deleted')); ?>">' + row.asset.serial + '</a></del></span>';
            }
    ...
    function trueFalseFormatter(value) {
        if ((value) && ((value == 'true') || (value == '1'))) {
            return '<i class="fas fa-check text-success"></i><span class="sr-only"><?php echo e(trans('general.true')); ?></span>';
        } else {
            return '<i class="fas fa-times text-danger"></i><span class="sr-only"><?php echo e(trans('general.false')); ?></span>';
        }
    }
    ...
    function emailFormatter(value) {
        if (value) {
            return '<a href="mailto:' + value + '" style="white-space: nowrap" data-tooltip="true" title="<?php echo e(trans('general.send_email')); ?>"><i class="fa-regular fa-envelope" aria-hidden="true"></i> ' + value + '</a>';
        }
    }
    ...
    function auditImageFormatter(value){
        if (value){
            return '<a href="' + value.url + '" data-toggle="lightbox" data-type="image"><img src="' + value.url + '" style="max-height: <?php echo e($snipeSettings->thumbnail_max_h); ?>px; width: auto;" class="img-responsive"></a>'
        }
    }
...
<?php $__env->stopPush(); ?><?php /**PATH /home2/flipperschool/inventory.flipperschool.com/resources/views/partials/bootstrap-table.blade.php ENDPATH**/ ?>